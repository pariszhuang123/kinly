...
ROUTE_CONSTANT_DEF = re.compile(r"static\s+const\s+(\w+)\s*=\s*['\"]([^'\"]+)['\"];")
ROUTE_USAGE_PATTERN = re.compile(r"GoRoute\s*\(\s*[\s\S]*?path:\s*([^,\s]+)")
...
def get_module_name(file_path):
    try:
        ...
        if not parts: return 'main.dart'
        top_level = parts[0]
        if top_level == 'main.dart': return 'main.dart'
        if top_level in CONFIG_MODULES:
            if len(parts) > 1: return f"{top_level}.{parts[1]}"
            return top_level
        if top_level == 'generated': return 'generated'
        if 'renderer' in top_level: return 'renderer'
        return top_level
    except ValueError:
        return None

def get_module_name_from_import(import_path):
...
def load_route_constants():
    paths_file = LIB_DIR / 'app' / 'router' / 'app_route_paths.dart'
    if not paths_file.exists(): return
    with open(paths_file, 'r', encoding='utf-8') as f:
        content = f.read()
    matches = ROUTE_CONSTANT_DEF.findall(content)
    for name, val in matches:
        route_constants[f"AppRoutePaths.{name}"] = val
...
def scan_files():
    ...
    for root, dirs, files in os.walk(LIB_DIR):
        for file in files:
            ...
            matches = ROUTE_USAGE_PATTERN.findall(content)
            for m in matches:
                val = m.strip()
                val = val.strip("'\"")
                if val in route_constants:
                    routes.append(route_constants[val])
                else:
                    routes.append(val)
...
def classify_layer(module_name):
    if module_name == 'main.dart': return 'main'
    if module_name == 'generated': return 'generated'
    parts = module_name.split('.')
    top = parts[0]
    if top in LAYER_ORDER: return top
    if 'renderer' in top: return 'renderer'
    return 'other'

def handle_output(path, content, check_mode):
...
def generate_di_graph_md(check_mode=False):
...
def generate_business_map(check_mode=False):
    if not capabilities_map: return True # Fail gracefully if loading failed
...
def generate_routes_md(check_mode=False):
    ...
    for r in unique_routes:
        if r == '/': continue
        node_id = sanitize_id("route_" + str(hash(r)).replace('-', ''))
    ...
    for r in unique_routes:
        ...
        parent = '/'
        best_len = 1
        for potential_parent in unique_routes:
            if potential_parent == r: continue
            if r.startswith(potential_parent):
                if len(potential_parent) > 1 and r[len(potential_parent)] != '/': continue
                if len(potential_parent) > best_len:
                    parent = potential_parent
                    best_len = len(potential_parent)
        
        parent_id = "root" if parent == '/' else sanitize_id("route_" + str(hash(parent)).replace('-', ''))
        child_id = sanitize_id("route_" + str(hash(r)).replace('-', ''))
    ...
    lines.append("```")
    return handle_output(DOCS_DIR / "routes.md", '\n'.join(lines), check_mode)

def generate_report_md(check_mode=False):
