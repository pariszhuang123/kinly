...
void main(List<String> args) {
  ...
  final arbPath = positional.isNotEmpty ? positional.first : 'lib/l10n/intl_en.arb';
  ...
  final invalidReferences = scanResult.references
  ...
  final nonEnExtras = enforceNonEn ? _findNonEnExtras(arbFile.parent, arbFile.path, canonicalKeys) : [];
  ...
  if (invalidReferences.isNotEmpty) {
    hasFailure = true;
    stderr.writeln('Invalid localization references (missing from EN): ${invalidReferences.length}');
...
void _printUsage() {
  stdout.writeln('Usage: dart run tool/l10n_integrity_check.dart [<path-to-en-arb>] [--enforce-non-en]');
...
List<_Reference> _extractReferences(String content, String path) {
  ...
  for (final pattern in _referencePatterns) {
    for (final match in pattern.allMatches(content)) {
      ...
      refs.add(_Reference(key: key, path: path, line: line));
    }
  }

  for (final alias in aliases) {
    final aliasPattern =
        RegExp('\\b${RegExp.escape(alias)}\\s*\\.\\s*([A-Za-z0-9_]+)');
...
Set<String> _extractAliases(String content) {
  ...
  final aliasPattern = RegExp(
    r'\b(?:late\s+final|final|var|const|late)?\s*(\w+)\s*=\s*(?:S\.of\([^;]+?\)|S\.current|context\.l10n)\s*;',
    multiLine: true,
    dotAll: true,
  );
...
List<_NonEnDrift> _findNonEnExtras(Directory arbDir, String enPath, Set<String> canonicalKeys) {
