...
- Active member: `EXISTS(SELECT 1 FROM membership m WHERE m.home_id = homes.id AND m.user_id = auth.uid() AND m.left_at IS NULL)`.
...
- membership (UPDATE left_at): no direct updates; only via `homes.leave` (self) or `members.kick` (owner-only).
...
- One active membership per user across homes (unique partial index on `(user_id)` WHERE `left_at IS NULL`).
